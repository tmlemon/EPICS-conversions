...
if str(SU.PVUtil.getString(pvs[1])) == 'restore' and go == 1:
    ...
    if not os.path.isfile(path+savFile) or savFile[-4:] != '.sav':
        if savFile == '':
            SU.ConsoleUtil.writeInfo('ERROR: No file to restore from found in\ current directory.')
        else:
            SU.ConsoleUtil.writeInfo('ERROR: File to restore from must exist.')
        sys.exit(1)
